...
exports.handler = async (event) => {
  ...
  if (action === 'add') {
    ...
    if (error) {
      if (error.code === '23505') return { statusCode: 409, body: 'Email already a manager' };
      console.error(error);
      return { statusCode: 500, body: 'Database error' };
    }
    return { statusCode: 200, body: 'Added as manager' };
  } else {
    // Prevent removing yourself?
    if (email === claims.email) return { statusCode: 400, body: 'You cannot remove yourself' };
    const { error } = await supabase.from('admins').delete().eq('email', email);
    if (error) {
      console.error(error);
      return { statusCode: 500, body: 'Database error' };
    }
    return { statusCode: 200, body: 'Removed from managers' };
  }
};
